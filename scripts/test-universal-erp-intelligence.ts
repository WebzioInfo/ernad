...
import { KenbyToolExecutorService } from '../backend/src/modules/ai/tools/kenby-tool-executor.service';
import { KenbyErpRegistryService } from '../backend/src/modules/ai/kenby-erp-registry.service';
import { KenbyLiveDataService } from '../backend/src/modules/ai/kenby-live-data.service';
import { GroqLlmService } from '../backend/src/modules/ai/llm/groq-llm.service';
...
async function runComprehensiveIntelligenceSuite() {
  ...
  await assertTest('Zero sales on August 2 returns 0 quantity and honest zero message', async () => {
  ...
  await assertTest('Live sales on August 10 returns exact 1000 quantity', async () => {
    const res = await toolExecutor.executeTool('get_sales_summary', { date: '2026-08-10' });
  ...
  await assertTest('list_vendors returns real vendor records', async () => {
  ...
  await assertTest('searchEntities resolves exact match for unique customer', async () => {
    const res = await erpRegistry.searchEntities('Sinan');
    return res.status === 'EXACT_MATCH' || res.status === 'HIGH_CONFIDENCE_MATCH';
  });

  await assertTest('searchEntities identifies multiple matches for ambiguous query', async () => {
    const res = await erpRegistry.searchEntities('Kenby');
  ...
  await assertTest('Ask AI: "August 10 sales എത്ര" -> Returns 1,000 cases', async () => {
    const res = await aiService.askQuestion('August 10 sales എത്ര');
  ...
  await assertTest('Ask AI: "എല്ലാ products ഏതൊക്കെയാണ്?" -> Lists products', async () => {
    ...
    const ans = res.answer.ml;
    return ans.includes('Kenby') || ans.includes('ഉൽപ്പന്നങ്ങൾ');
  });
