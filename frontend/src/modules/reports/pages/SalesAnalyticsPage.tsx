...
import { generateSalesPDF } from '../../../utils/pdfExport';
...
export default function SalesAnalyticsPage() {
  ...
  const exportPDF = async () => {
    ...
    try {
      await generateSalesPDF(dateRange, salesData);
    } finally {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4 bg-white/50 backdrop-blur-md p-2 rounded-[2rem] border border-slate-100 shadow-sm">
          ...
          <button
            ...
            className="p-5 bg-slate-900 text-white rounded-3xl hover:bg-slate-800 transition-all shadow-xl active:scale-95 flex items-center justify-center disabled:opacity-50"
          >
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={MOCK_REVENUE}>
                ...
                <Area type="monotone" dataKey="value" stroke="#1A9A91" strokeWidth={4} fillOpacity={1} fill="url(#colorRevenue)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
...
function KPICard({ label, value, trend, icon: Icon, color, negative }: any) {
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 shadow-lg ${color === 'emerald' ? 'bg-emerald-500 text-white shadow-emerald-100' :
      ...
      <div className="flex justify-between items-end">
        ...
        <div className={`flex items-center gap-1 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest ${negative ? 'bg-rose-50 text-rose-500' : 'bg-emerald-50 text-emerald-500'
          }`}>
