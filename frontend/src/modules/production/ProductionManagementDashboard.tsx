...
const KPICard = ({ label, value, subValue, icon: Icon, color, trend, delay = 0 }: any) => (
  <motion.div
    ...
    transition={{ delay }}
    className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-md rounded-3xl p-6 relative overflow-hidden group hover:bg-white/[0.04] transition-all"
  >
    ...
    <div className="flex items-baseline gap-2">
      <h4 className="text-4xl font-mono font-black text-white tracking-tighter tabular-nums">
        {value}
      </h4>
      {subValue && (
...
export default function ProductionManagementDashboard() {
  ...
  if (loadingBatches) return (
    <div className="h-screen w-full bg-[#02040a] flex flex-col items-center justify-center relative overflow-hidden">
      <TechnicalBackground />
      <motion.div
        animate={{ rotate: 360 }}
      ...
      />
      <p className="text-slate-500 font-mono text-[10px] uppercase tracking-[0.4em] animate-pulse">Initializing Command Center...</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#02040a] text-slate-300 selection:bg-indigo-500 selection:text-white">
      <TechnicalBackground />
      ...
      <main className="relative z-10 p-8 lg:p-12 space-y-12 max-w-[1800px] mx-auto">
        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          ...
          >
            ...
            <p className="text-slate-500 font-bold uppercase tracking-[0.3em] text-[10px] ml-1 flex items-center gap-3">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]" />
              System Status: Nominal • Factory ID: ERN-KL01
            </p>
          </motion.div>

          <div className="flex items-center gap-6 bg-white/[0.02] border border-white/5 backdrop-blur-xl p-4 rounded-3xl">
            <div className="text-right px-6 border-r border-white/5">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Active Batches</p>
              <p className="text-3xl font-black text-white font-mono">{factoryLive?.activeBatches?.length || 0}</p>
            </div>
            <div className="text-right px-6 border-r border-white/5">
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Global Efficiency</p>
              <p className="text-3xl font-black text-emerald-500 font-mono">92.4%</p>
            </div>
            ...
            <button
              onClick={() => { }}
              className="p-4 bg-white/5 text-slate-400 border border-white/10 rounded-2xl hover:bg-white/10 transition-all group active:scale-95"
            >
        ...
        <div className="grid grid-cols-12 gap-10">
          ...
          <div className="col-span-12 lg:col-span-8 space-y-8">
            <div className="bg-white/[0.02] border border-white/[0.05] rounded-[3rem] overflow-hidden backdrop-blur-md">
              <div className="p-8 border-b border-white/[0.05] flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="h-12 w-64 bg-black/40 border border-white/10 rounded-2xl pl-12 pr-6 text-sm font-bold text-white outline-none focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 transition-all"
                    />
                  </div>
                  ...
                  <select
                    ...
                    className="h-12 bg-black/40 border border-white/10 rounded-2xl px-6 text-[10px] font-black uppercase tracking-widest text-slate-400 outline-none focus:border-indigo-500/50 transition-all"
                  >
                      ...
                      {filteredBatches.map((batch: any, i: number) => (
                        ...
                        >
                          <td className="px-8 py-6">
                            <p className="text-sm font-black text-white group-hover:text-indigo-400 transition-colors font-mono tracking-tight">{batch.batchCode}</p>
                            <p className="text-[10px] font-bold text-slate-600 mt-1 uppercase tracking-tighter">ID: {batch.id.slice(0, 8)}</p>
                          </td>
                          <td className="px-8 py-6">
                            <p className="text-xs font-black text-slate-300 uppercase">{batch.lineName || 'LINE 01'}</p>
                            <p className="text-[10px] font-bold text-slate-500 mt-1 truncate max-w-[150px]">{batch.productName}</p>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-6">
                              <div>
                                <p className="text-xs font-black text-white tabular-nums font-mono">{(batch.actualQuantity || 0).toLocaleString()}</p>
                                <p className="text-[8px] font-black text-slate-600 uppercase">Yield</p>
                              </div>
                              <div>
                                <p className="text-xs font-black text-rose-500 tabular-nums font-mono">{(batch.rejectionTotal || 0).toLocaleString()}</p>
                                <p className="text-[8px] font-black text-slate-600 uppercase">Rejects</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-8 py-6">
                            <p className="text-[10px] font-bold text-slate-400 font-mono">{format(new Date(batch.startTime), 'MMM dd, HH:mm')}</p>
                            <p className="text-[8px] font-black text-slate-600 uppercase mt-1 tracking-widest">Clock In</p>
                          </td>
                          <td className="px-8 py-6">
                            <div className={cn(
              ...
              {filteredBatches.length === 0 && (
                <div className="py-24 text-center">
                  ...
                  <p className="text-slate-600 text-[10px] font-bold mt-2 uppercase tracking-widest">The request returned an empty industrial set.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-10">
            ...
            {/* Live Audit Stream */}
            <section className="bg-white/[0.02] border border-white/[0.05] rounded-[3rem] p-8 backdrop-blur-md relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center mb-10 relative z-10">
                ...
                <span className="px-3 py-1 bg-amber-500/10 text-amber-500 rounded-full text-[9px] font-black uppercase tracking-widest">Audit Live</span>
              </div>
              ...
              <div className="space-y-6 relative z-10">
                ...
                {[
                  ...
                  <motion.div
                    ...
                    className="p-5 bg-black/40 border border-white/[0.03] rounded-2xl hover:bg-white/[0.02] transition-all cursor-pointer group/item"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <p className="text-xs font-black text-white group-hover/item:text-indigo-400 transition-colors uppercase tracking-tight">{event.action}</p>
                      <span className="text-[8px] font-black text-slate-600 font-mono">{event.time}</span>
                    </div>
                    <div className="flex items-center gap-2">
              ...
              <button className="w-full mt-10 py-5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 text-slate-400 hover:text-white">
                View Security Ledger
              </button>
            </section>

            {/* Industrial Visualization: Line Efficiency Comparison */}
            <section className="bg-slate-900 border border-white/[0.05] rounded-[3rem] p-8 shadow-2xl relative overflow-hidden group">
                ...
                {lines?.slice(0, 3).map((line: any, i: number) => (
                  <div key={line.id} className="space-y-2">
                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                      <span>{line.name}</span>
                      <span className="text-white font-mono">{Math.floor(80 + Math.random() * 15)}%</span>
                    </div>
              ...
              <div className="mt-10 p-6 bg-white/[0.03] rounded-2xl border border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Prediction Model</p>
                    <p className="text-xs font-bold text-white leading-tight">Current velocity suggests 98.4% target fulfillment.</p>
                  </div>
                </div>
              </div>
            </section>
      ...
      <footer className="relative z-10 border-t border-white/5 mt-12 py-10">
        <div className="max-w-[1800px] mx-auto px-12 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
            <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Operator Sessions: <span className="text-white">ACTIVE</span></div>
            <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Network Latency: <span className="text-emerald-500 font-mono">14ms</span></div>
          </div>
          <p className="text-[9px] font-black text-slate-700 uppercase tracking-[0.4em]">Eranad Beverages MES • Industrial Grade Platform</p>
        </div>
      </footer>
    </div>
  );
