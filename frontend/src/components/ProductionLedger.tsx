    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('Batch No') || line.includes('Batch no')) {
          ...
          let wastageCount = 0;
          if (rejStr.includes('=')) {
            wastageCount = parseInt(rejStr.split('=')[1].trim());
          } else {
            // Extract all numbers and sum them
            const nums = rejStr.match(/\d+/g);
            wastageCount = nums ? nums.reduce((acc, n) => acc + parseInt(n), 0) : 0;
          }
          ...
          <div className="space-y-3 mb-10">
            {entries.map((entry, idx) => (
              <div key={idx} className="flex items-center gap-6 p-5 bg-slate-50/50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-300 group">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${entry.station === 'BLOWING' ? 'bg-blue-600 text-white' :
